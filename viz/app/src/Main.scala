...
object Main:
  ...
  lazy val pcaLoad: List[PcaLoad] = Globals.elmixPcaLoadings.toList.map { d =>
    PcaLoad(d.zone.toString, d.pc.asInstanceOf[Double].toInt,
            d.kraftslag.toString, d.loading.asInstanceOf[Double])
  }
  lazy val pcaScores: List[PcaScore] = Globals.elmixPcaScores.toList.map { d =>
    PcaScore(d.zone.toString, d.yr.asInstanceOf[Double].toInt,
             d.pc1.asInstanceOf[Double], d.pc2.asInstanceOf[Double])
  }
  ...
  def screeOption(): js.Any =
    val grids = zones.zipWithIndex
    obj(
      title = js.Array(grids.map { (z, i) => obj(text = z.replace("_", ""),
        left = s"${8 + col(i) * 48 + 19}%", top = 38 + row(i) * RowH,
        textAlign = "center", textStyle = obj(fontSize = 13)) }*),
      grid = js.Array(grids.map { (_, i) => obj(left = s"${8 + col(i) * 48}%",
        width = "38%", top = 64 + row(i) * RowH, height = 180) }*),
      xAxis = js.Array(grids.map { (_, i) => obj(`type` = "category", gridIndex = i,
        data = js.Array(Pcs.map(p => s"PC$p")*)) }*),
      yAxis = js.Array(grids.map { (_, i) => obj(`type` = "value", gridIndex = i,
        min = 0, max = 1, name = if i == 0 then "förklarad andel" else "") }*),
      series = js.Array(grids.map { (z, i) => obj(`type` = "bar",
        xAxisIndex = i, yAxisIndex = i, itemStyle = obj(color = "#5470c6"),
        data = js.Array(Pcs.map(p =>
          pcaExp.find(e => e.zone == z && e.pc == p).map(_.explained).getOrElse(0.0))*),
        label = obj(show = true, position = "top", fontSize = 9,
  ...
  def loadingsOption(): js.Any =
    ...
    obj(
      title = js.Array(grids.map { (z, i) => obj(text = z.replace("_", ""),
        left = s"${8 + col(i) * 48 + 17}%", top = 38 + row(i) * RowH,
        textAlign = "center", textStyle = obj(fontSize = 13)) }*),
      grid = js.Array(grids.map { (_, i) => obj(left = s"${10 + col(i) * 48}%",
        width = "32%", top = 64 + row(i) * RowH, height = 180) }*),
      xAxis = js.Array(grids.map { (_, i) => obj(`type` = "category", gridIndex = i,
        data = js.Array(Pcs.map(p => s"PC$p")*)) }*),
      yAxis = js.Array(grids.map { (_, i) => obj(`type` = "category", gridIndex = i,
        data = js.Array(Fuels.map(fuelAbb)*)) }*),
      visualMap = obj(min = -1, max = 1, dimension = 2, calculable = false,
        orient = "horizontal", left = "center", bottom = 0,
        inRange = obj(color = js.Array("#c0392b", "#f7f7f7", "#2c7fb8"))),
      series = js.Array(grids.map { (z, i) => obj(`type` = "heatmap",
        ...
        label = obj(show = true, fontSize = 8,
  ...
  def biplotOption(zone: String): js.Any =
    ...
    val scatter = yrs.map { y =>
      obj(name = y.toString, `type` = "scatter", symbolSize = 4,
    ...
    val vectors = Fuels
      ...
      .map { (f, l1, l2) =>
        obj(name = f, `type` = "line", symbol = js.Array("none", "arrow"),
          symbolSize = 9, silent = true, lineStyle = obj(color = "#333", width = 1.5),
    ...
    obj(
      title = obj(text = s"Biplot – ${zone.replace("_", "")} (PC1 vs PC2, dagsmedel)", left = "center"),
  ...
  def appElement(): HtmlElement =
    ...
    div(
      ...
      div(
        ...
        zones.map { z =>
          label(
            input(typ := "radio", nameAttr := "biplot-zone",
